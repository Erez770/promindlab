...
const staticData = [
  { rating: 5, initials: 'АП', gradient: 'from-blue-500 to-violet-500' },
  { rating: 5, initials: 'МИ', gradient: 'from-pink-500 to-rose-500' },
  { rating: 5, initials: 'ДК', gradient: 'from-emerald-500 to-teal-500' },
  { rating: 5, initials: 'АС', gradient: 'from-amber-500 to-orange-500' },
  { rating: 5, initials: 'ИВ', gradient: 'from-cyan-500 to-blue-500' },
  { rating: 5, initials: 'ЕН', gradient: 'from-violet-500 to-purple-500' },
];
...
function TestimonialCard({ t, index }: { t: Testimonial; index: number }) {
  return (
    ...
    >
      ...
      {/* Stars */}
      <div className="flex gap-0.5" role="img" aria-label={`${t.rating}/5`}>
...
export default function Testimonials() {
  ...
  const translated = t.raw('testimonials') as Array<{name: string; role: string; company: string; text: string; project: string; initials: string}>;
  const testimonials = staticData.map((s, i) => ({ ...s, ...translated[i] }));
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-light mb-6">
            <div className="flex gap-0.5" role="img" aria-label={t('ratingAria')}>
            ...
            <span className="text-[12px] font-medium text-muted">{t('badge')}</span>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {testimonials.map((testimonial, i) => (
            <TestimonialCard key={i} t={testimonial} index={i} />
          ))}
        </div>
      </div>
    </section>
